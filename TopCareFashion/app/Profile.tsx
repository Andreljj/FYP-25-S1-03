...
const ProfileScreen = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const response = await fetch("https://topcare-fashion-backend.onrender.com/api/user/me/seller", {
  ...
  const fetchReviews = useCallback(async () => {
    try {
      const token = await AsyncStorage.getItem('token');
      const sellerId = params?.userId || profileData._id;
  ...
  }, [params?.userId, profileData._id]); // Dependencies for useCallback
  ...
  useEffect(() => {
    const fetchUserListings = async () => {
      try {
        const token = await AsyncStorage.getItem('token');
        const response = await fetch("https://topcare-fashion-backend.onrender.com/api/listings/mine", {
          headers: {
            Authorization: `Bearer ${token}`
          }
        });
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              const response = await fetch(`https://topcare-fashion-backend.onrender.com/api/listings/${listingId}`, {
                method: "DELETE",
                headers: {
                  Authorization: `Bearer ${token}`,
                },
              ...
              if (!response.ok) {
                ...
                Alert.alert("Error", "Failed to delete listing from server.");
                return;
              }
  
              // Remove from state
              const updatedListings = yourListings.filter(listing => listing._id !== listingId);
              setYourListings(updatedListings);
              await AsyncStorage.setItem("userListings", JSON.stringify(updatedListings));
  ...
  >
    {/* Absolute-positioned options button inside the card */}
    <TouchableOpacity
      style={styles.optionsButton}
      onPress={() =>
        setShowOptionsModal(showOptionsModal === item._id ? null : item._id)
      }
    >
  ...
  {/* Options Modal outside for better zIndex positioning */}
  {showOptionsModal === item._id && (
...
const styles = StyleSheet.create({
  ...
  optionsButton: {
    ...
    top: 10,
    padding: 5,
    backgroundColor: 'rgba(234, 234, 234, 0.92)',
    borderRadius: 20,
    zIndex: 10,
  },
  listingImage: {
